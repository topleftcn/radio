				...
				<form name="myform" id="myform" method="post">
					<div class="result-title">
						<div class="result-list">
							<a href="/radio/admin/indexinsert.jsp"><i class="icon-font"></i>新增内容</a>
						</div>
					</div>
					<div class="result-content">
						<table class="result-tab" width="100%">
							...
							<c:if test="${allIndexSize-1>=0 }">
							<c:forEach begin="0" step="1" end="${allIndexSize-1 }" var="i">
								<tr>
									<td class="tc"><input name="${allIndex[i].id }"
										type="checkbox"></td>
									<td>${allIndex[i].id }</td>
									<td>${allIndex[i].english }</td>
									<td>${allIndex[i].gonggao }</td>
									<td>${allIndex[i].inserttime }</td>
									<td><a class="link-update"
										href="/radio/deleteindex.do?id=${allIndex[i].id }">删除</a></td>
								</tr>
						...
						<div class="list-page">
							<c:if test="${pages!=1 }">
								<a href="/radio/indexset.do?pages=${pages-2 }">上一页</a>
							</c:if>
							&nbsp;${pages } / ${totalPages }&nbsp;
							<c:if test="${pages < totalPages }">
								<a href="/radio/indexset.do?pages=${pages }">下一页</a>
							</c:if>
						</div>
					</div>
				</form>
