				...
				<form name="myform" id="myform" method="post">
					<div class="result-title">
						<div class="result-list">
							<a href="/radio/admin/newsinsert.jsp"><i class="icon-font"></i>新增新闻</a>
						</div>
					</div>
					<div class="result-content">
						<table class="result-tab" width="100%">
							...
							<c:if test="${allNewsSize-1 >=0}">
								<c:forEach begin="0" step="1" end="${allNewsSize-1}" var="i">
									<tr>
										<td class="tc"><input name="${allNews[i].id }"
											type="checkbox"></td>
										<td>${allNews[i].id }</td>
										<td><c:if test="${allNews[i].type == 0}">校内新闻</c:if> <c:if
												test="${allNews[i].type == 1}">校外新闻</c:if></td>
										<td>${allNews[i].title }</td>
										<td>${allNews[i].inserttime }</td>
										<td><a class="link-update"
											href="/radio/deletenews.do?id=${allNews[i].id }">删除</a></td>
									</tr>
						...
						<div class="list-page">
							<c:if test="${pages!=1 }">
								<a href="/radio/newsset.do?pages=${pages-2 }">上一页</a>
							</c:if>
							&nbsp;${pages } / ${totalPages }&nbsp;
							<c:if test="${pages < totalPages }">
								<a href="/radio/newsset.do?pages=${pages }">下一页</a>
							</c:if>
						</div>
					</div>
				</form>
